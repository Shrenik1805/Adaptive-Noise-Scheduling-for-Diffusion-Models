...
\begin{abstract}
Diffusion models typically use a fixed noise schedule shared across all samples, despite large variation in semantic complexity across classes. We introduce \textbf{Adaptive Noise Scheduling} (ANS), a class-conditional framework that learns $\beta_t(x)$ using a dedicated ScheduleNet and jointly optimizes generation fidelity, schedule efficiency, and temporal smoothness. ScheduleNet receives a class embedding and timestep encoding, then predicts bounded per-step noise levels with differentiable constraints enforcing valid diffusion dynamics. Our objective augments standard $\epsilon$-prediction loss with (i) an efficiency term that discourages unnecessary cumulative noise and (ii) a smoothness regularizer stabilizing reverse trajectories. On CIFAR-10, ANS is designed to improve the quality--latency frontier by matching or improving FID at lower denoising cost versus fixed linear/cosine schedules and DDIM baselines. We further provide per-class analyses showing schedule diversity aligned with class-dependent generation difficulty, supporting the hypothesis that adaptive schedules can better allocate diffusion budget than one-size-fits-all noise policies.
\end{abstract}
...
We propose \textbf{Adaptive Noise Scheduling (ANS)}, where a neural scheduler outputs class-specific $\beta_t(x)$ and is trained jointly with the diffusion denoiser.
...
Let $g_\phi$ denote ScheduleNet. Given class embedding $\mathbf{c}(x)$ and timestep $t$, we predict
\begin{align}
\beta_t(x) = g_\phi\!\left(\mathbf{c}(x), \text{PE}(t/T)\right), \quad \beta_t(x)\in[\beta_{\min},\beta_{\max}],
\end{align}
with learnable bounds satisfying $0<\beta_{\min}<\beta_{\max}\le 0.02$.
Then
\begin{align}
\bar{\alpha}_t(x) = \prod_{i=1}^{t}\left(1-\beta_i(x)\right).
\end{align}
...
\paragraph{Why $\mathcal{L}_{\text{efficiency}}$ encourages fewer steps.}
For fixed denoiser capacity, large cumulative noise implies larger reverse correction magnitude and typically more denoising iterations to reach target fidelity. Minimizing $\sum_t \beta_t(x)$ reduces average corruption budget, lowering the effective reverse integration burden. Empirically, this shifts the FID--latency frontier toward fewer-step operating points.

\section{Experiments}
\subsection{Setup}
Dataset: CIFAR-10 ($32\times32$). Base denoiser: class-conditional U-Net. ScheduleNet: residual MLP with sinusoidal timestep encoding.

\subsection{Baselines}
We compare ANS to fixed linear schedule, fixed cosine schedule~\cite{nichol2021improved}, and DDIM acceleration~\cite{song2020ddim}.

\subsection{Metrics}
Primary metrics: FID, wall-clock sampling time, per-class speedup ratio, and schedule diversity (pairwise class schedule distance).

\section{Results}
\begin{table}[t]
\centering
\caption{Main quality/efficiency comparison on CIFAR-10.}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[t]
\centering
\caption{Per-class adaptive-vs-fixed summary (CIFAR-10).}
\begin{tabular}{lccc}
\toprule
...
\section{Conclusion}
Adaptive class-conditional noise scheduling is a simple, differentiable extension of DDPM training that directly targets the fidelity-efficiency tradeoff. By replacing fixed schedules with learned $\beta_t(x)$ and regularizing for smooth, low-noise trajectories, ANS provides a principled path to faster diffusion sampling while preserving quality.
